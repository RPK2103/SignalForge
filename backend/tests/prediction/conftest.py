"""Fixtures for Delivery Prediction tests (Phase 3 Prompt 4)."""
...
def temp_database_url(tmp_path: Path) -> str:
    return f"sqlite:///{(tmp_path / 'prediction.db').as_posix()}"
...
@pytest.fixture
def projected_novabank(seeded_db: str, novabank_tenant: TenantContext) -> str:
...
@pytest.fixture
def client(projected_novabank: str) -> Generator[TestClient, None, None]:
